...
#
# This mirrors spattoo-core/.githooks/_lib.sh. Separate repos cannot share a file
# without a monorepo, so a per-repo copy is the same accepted trade-off as the
# commit-msg gate that every repo carries. Keep them in step when editing either.
